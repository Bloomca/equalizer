...
export default function Controls(sources) {
  ...
  const vdom$ = sources.player$.map(state => {
    const tracks = state.playlist || [];
    const currentIndex = state.track
      ? tracks.findIndex(x => x.id === state.track.id)
      : -2;
      ...
      DOM: xs.of(volumeValue).map(value => {
        ...
        return div(`.${styles.container}.mui--divider-bottom`, [
          div(`.${styles.background}`, {
            style: { backgroundImage: `url(${state.track.waveform_url})` }
          }),
          div(`.${styles.firstRow}`, [
            div(`.${styles.logoContainer}`, [imgMarkup]),
            div(`.${styles.infoContainer}`, [
              h3(`.${styles.title}`, [
                ...
                a(
                  `.${styles.itemLink}`,
                  {
                    attrs: { target: "_blank", href: state.track.permalink_url }
                  },
                  ["SoundCloud"]
                )
              ]),
              p(`.${styles.description}`, [state.track.description]),
              div(`.mui--text-dark-hint`, [
                ...
                a(
                  `.link`,
                  {
                    attrs: {
                      target: "_blank",
                      href: state.track.user.permalink_url
                    }
                  },
          ...
          div(`.${styles.buttonsContainer}`, [
            div(`.${styles.buttons}`, [
              button(".prev.mui-btn.mui-btn--primary", ["PREV"]),
              state.paused ? playButton : pauseButton,
              button(".next.mui-btn.mui-btn--primary", ["NEXT"]),
              nextTrackMarkup
            ]),
            div(`.${styles.volumeContainer}`, [
              div(`.${styles.volumeTitle}.mui--text-dark-hint`, ["Volume: "]),
              input(`.${styles.volume}`, {
