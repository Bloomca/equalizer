...
export default function(sources) {
  const streams$ = sources.player$
    .map(player => {
      const sliders = (player.filters || []).map(({ frequency, gain }) => {
        const customClass = `slider-${frequency.value}`;
        const gain$ = sources.DOM.select(`.${customClass}`).events('input')
          .map(ev => ({ type: 'frequency', value: ev.target.value, frequency: frequency.value }));

        const sliderAttrs = { orient: 'vertical', type: 'range', min: -10, max: 10, value: gain.value };
        return {
          DOM: div(`.${styles.sliderContainer}`, [
            input(`.${styles.slider}.${customClass}`, { attrs: sliderAttrs }),
            div(`.${styles.sliderFrequency}`, [
