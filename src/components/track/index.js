...
export default function Track(sources) {
  const streams$ = xs.of(sources.track).map(track => {
    ...
    const duration = x.duration
      ? div(`.${styles.duration}`, [formatDuration(x.duration / 1000)])
      : null;
    return {
      DOM: div(
        `.track.${customClass}.mui--divider-bottom.${styles.container}.${
          x.id === sources.active ? styles.active : ""
        }`,
        [h4(`.${styles.title}`, [x.title]), duration]
      ),
