...
export default function Tabs(sources) {
  ...
  const container$ = sources.DOM.select(`.${styles.spectrogramInnerContainer}`).elements();
  ...
  const spectrogramComponent = Spectrogram({ DOM: sources.DOM, state$: sources.player$, container$ });
  const equalizerComponent = Equalizer({ DOM: sources.DOM, player$: sources.player$ });
  const vtree$ = xs.combine(
    sources.player$, state$, searchComponent.value, searchComponent.DOM, tracksComponent.DOM,
    spectrogramComponent.DOM, equalizerComponent.DOM
  ).map(([player, state, value, searchDOM, tracksDOM, spectrogramDOM, equalizerDOM]) => {
      const tabs = player.playing
        ? div(`.${styles.tabsContainer}`, [
            ul(`.mui-tabs__bar.${styles.tabs}`, [
              li(`.tab-eq.${state === 'equalizer' ? 'mui--is-active' : ''}.${styles.tab}`, [
                'Equalizer'
              ]),
              li(`.tab-songs.${state === 'songs' ? 'mui--is-active' : ''}.${styles.tab}`, [
                'Playlist'
              ])
            ])
          ]) : null;
        ...
        content = div('.my-content', [
          div(`.${styles.dataContainer}`, [
            div(`.${styles.spectrogramContainer}`, [
              div(`.${styles.spectrogramInnerContainer}`, [
                spectrogramDOM
              ])
            ]),
            div(`.${styles.equalizerContainer}`, [
              equalizerDOM
            ])
          ])
        ]);
